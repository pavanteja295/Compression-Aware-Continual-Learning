...
class SVD_Conv2d(torch.nn.Module):
    def __init__(self,input_channel,output_channel,kernel_size,stride = 1,padding = 0,
                bias = False,SVD_only_stride_1 = False,decompose_type = 'channel', rank=None):
        """
        ...
        if not SVD_only_stride_1 or self.stride==1:
            if self.decompose_type == 'channel':
                ####################### CHANGE THIS LATERRRRRRRRRRRRR rank if rank  else min(input_channel*kernel_size*kernel_size, output_channel) #
                self.r = rank if rank else int((input_channel*kernel_size*kernel_size*output_channel) / (output_channel + (input_channel*kernel_size*kernel_size)) ) #min(output_channel,input_channel*kernel_size*kernel_size)
                print(f'Now : {self.r},  Then :  {output_channel,input_channel*kernel_size*kernel_size}' )
                ...
                # print(self.N.shape, self.C.shape, self.Sigma.shape)
...
class Net_SVD(nn.Module):
    """Small architechture"""
    def __init__(self,num_classes=10, rank=[], size=2*2):
        super(Net_SVD, self).__init__()
        # self.act=OrderedDict()
        self.conv1 = SVD_Conv2d(3, 64, 3, bias=False, rank=rank[0] if rank else None)
        self.conv2 = SVD_Conv2d(64, 64, 3, bias=False,rank=rank[1] if rank else None)
        self.drop_outA = nn.Dropout(0.15)
        self.conv3 = SVD_Conv2d(64, 128, 3, bias=False, rank=rank[2] if rank else None)
        self.conv4 = SVD_Conv2d(128, 128,3, bias=False, rank=rank[3] if rank else None)
        self.drop_outB = nn.Dropout(0.15)
        # self.conv5 = SVD_Conv2d(128,256,2, bias=False)
        self.conv6 = SVD_Conv2d(128,256,2, bias=False, rank=rank[4] if rank else None)
        self.size = size

        # for imagenet its 8*8
        # 32x32 its 2*2
        # self.last = nn.Linear(256*8*8, num_classes)
        self.last = nn.Linear(256*self.size, num_classes)
    def logits(self, x):
        x = self.last(x)
        return x
