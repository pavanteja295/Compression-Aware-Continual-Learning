...
class BasicBlock(nn.Module):
	...
	def __init__(self, inplanes, planes,  stride=1,downsample=None,SVD_only_stride_1 = False,decompose_type = 'channel', track_running_stats = True):
...
class Bottleneck(nn.Module):
	...
	def __init__(self, inplanes, planes, stride=1, downsample=None,SVD_only_stride_1 = False,decompose_type = 'channel', track_running_stats=True):
		...
		self.bn2 = nn.BatchNorm2d(planes, track_running_stats=track_running_stats)
		self.conv3 = SVD_Conv2d(planes, planes * 4, kernel_size=1, bias=False,SVD_only_stride_1 = SVD_only_stride_1,decompose_type = decompose_type)
...
class ResNet(nn.Module):
	...
	def __init__(self, depth=20, num_classes=1000,SVD_only_stride_1 = False,decompose_type = 'channel', track_running_stats=True):
		...
		self.inplanes = 16
		#input : 32x32x3
		self.conv1 = SVD_Conv2d(3, 16, kernel_size=3, padding = 1,bias=False,SVD_only_stride_1 = SVD_only_stride_1,decompose_type = decompose_type)#32x32x16
		self.bn1 = nn.BatchNorm2d(16, track_running_stats=track_running_stats)
		self.relu = nn.ReLU(inplace=True)
		self.layer1 = self._make_layer(block, 16, n, track_running_stats=track_running_stats)#32x32x16
		self.layer2 = self._make_layer(block, 32, n, stride=2, track_running_stats=track_running_stats)
		self.layer3 = self._make_layer(block, 64, n, stride=2, track_running_stats=track_running_stats)
	...
	def _make_layer(self, block, planes, blocks, stride=1, track_running_stats=False):
		...
		layers = []
		layers.append(block(self.inplanes, planes, stride, downsample,SVD_only_stride_1 = self.SVD_only_stride_1,decompose_type = self.decompose_type,track_running_stats=track_running_stats ))
		self.inplanes = planes * block.expansion

		for i in range(1, blocks):
			layers.append(block(self.inplanes, planes,SVD_only_stride_1 = self.SVD_only_stride_1,decompose_type = self.decompose_type, track_running_stats=track_running_stats))
